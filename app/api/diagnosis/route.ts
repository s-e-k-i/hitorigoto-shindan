...
1994年24歳で独立。様々なビジネスやどん底経験を経て、ひとりでやることを決意。独立から31年、ひとり起業家（マイクロプレナー）として活動中。
...
・ブログ・無料レポート・メルマガ（ブログで集客→無料レポートを配布→メルマガ登録という流れで読者を獲得。メルマガ10万部、ブログ100万ブログ中9位）
・コンサルティング（月収100万円を達成）
...
・AIツール開発（非エンジニア・プログラミング未経験ながらAIを使いひとりビジネス向けWebアプリを開発・公開）
...
・「何をビジネスにすればいいかわからない」→ 即金フリータイプを上位に（まず動く段階）
...
【関達也の実績に関する正確な数字（これ以外の数字を使わないこと）】
・メルマガ読者：最終的に10万人
・ブログ：Livedoorブログランキングで100万ブログ中9位を獲得
  ※これは「ブログの数が100万個あるランキングでの順位」であり、「読者数が100万人」という意味ではない。絶対に読者数と混同しないこと。
・19ヶ月で1億円達成
・教材1本で8,400万円以上の売上
・累計3,000名超をサポート
...
・上から目線の言い方
・上記「正確な数字」以外の具体的な数字・人数・金額・件数をコメント内で新たに作り出すこと
・上記の数字を別の意味に言い換えること（例：ランキング順位を読者数に変換する、など）
